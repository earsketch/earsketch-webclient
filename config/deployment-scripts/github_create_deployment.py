...
if len(sys.argv) < 5:
    ...
    print("Usage: github_create_release.py <GIT_USER> <GITHUB_TOKEN> <GIT_COMMIT_SHA> <PULL_REQUEST_NUMBER>")
    exit(1)
...
r = requests.post(url + "deployments", headers=headers, auth=auth, json=createDeploymentParams)
