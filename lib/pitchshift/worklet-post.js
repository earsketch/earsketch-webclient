...
class PitchshiftProcessor extends AudioWorkletProcessor {
    ...
    constructor() {
        super()
        console.log("constructing pitchshifter")
        // TODO: any way to get initial channel count here? (I suspect not.)
    ...
    process(inputs, outputs, parameters) {
        ...
        while (this.shifters.length > inputs[0].length) {
            console.log(`freeing shifter for missing channel; ${this.shifters.length} -> ${this.shifters.length - 1}`)
            this.shifters.pop()
        }
        while (this.shifters.length < inputs[0].length) {
            console.log(`adding shifter for new channel; ${this.shifters.length} -> ${this.shifters.length + 1}`)
