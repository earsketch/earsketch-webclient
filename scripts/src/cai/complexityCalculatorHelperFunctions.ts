...
export function getCallReturn(callingNode: any) {
    if (callingNode._astname === "Call") { // just return null if this is the wrong kind of node.
        ...
        const functionCalled = getFunctionObject(functionName)
...
export function performListOp(callingNode: any): any {
        ...
        if (opToPerform != null) {
            ...
            if (opToPerform === "copyWithin") {
                ...
                if (callingNode.args.length > 1) {
                    const startNode = retrieveFromList(callingNode.args[1])
                ...
                if (callingNode.args.length > 2) {
                    const endNode = retrieveFromList(callingNode.args[2])
                ...
                const sliceToCopy = listToUse.slice(startVal, endVal)
                const newList = []
            ...
            if (opToPerform === "push") {
                ...
                if (pushArg != null) {
                    const returnList = listToUse.slice(0)
    ...
    function pythonOp(callingNode: any) {
        ...
        if (opToPerform != null) {
            ...
            if (opToPerform === "remove") {
                let indexToRemove = -1
                const itemToRemove = callingNode.args[0]
    ...
    function pythonSortNestedLists(allListsInList: string[]) {
        ...
        const listsWithin = []
        let listsSorted = []
        const intSorted = []; const floatSorted = []; let listSorted = []; const stringSorted = []
...
export function isCallAStrOp(node: any) {
    ...
    if (thisNode != null && thisNode._astname === "Call") {
        if ("id" in thisNode.func) {
            ...
            const funcName = thisNode.func.attr.v
...
export function addOpToList(opToAdd: any, opList: any[], lineno: number) {
    ...
    for (const p in ccState.getProperty("loopLocations")) {
...
export function opsBeforeLine(opList: any[], lineNumber: number, funcOrVar: string, funcOrVarObject: any) {
    const opsBefore = [] // initialize return value
    ...
    if (funcOrVar === "var") {
        ...
        const modFuncs = []
        for (const i in funcOrVarObject.modifyingFunctions) {
            // find the function object in userFunctions
            let funcObj = null
            for (const p in ccState.getProperty("userFunctionReturns")) {
        ...
        for (const i in opList) {
            if (!opsBefore.includes(opList[i].op)) {
                let isBefore = false
                for (const p in opList[i].lines) {
                    for (const j in modFuncs) {
        ...
        const containedFuncs = []
        for (const i in ccState.getProperty("userFunctionReturns")) {
            if (ccState.getProperty("userFunctionReturns")[i].callsTo != null) {
                for (const p in ccState.getProperty("userFunctionReturns")[i].callsTo) {
        ...
        for (const i in containedFuncs) {
            for (const p in containedFuncs[i].opsDone) {
        ...
        if (binOp.left.includes(":")) {
            ...
            if (binOp.left.includes(":")) {
                ...
                if (binOp.left.startsWith("var")) {
                    ...
                    const leftVar = getVariableObject(varName)
                    if (leftVar != null) {
                    ...
                    const binOpFunc = getFunctionObject(funcName)
                    if (binOpFunc != null) {
...
export function listTypesWithin(node: Node, typesWithin: string[], inputIndexingObj: any, opList: any[]) {
    ...
    if (thisNode != null) {
        ...
        if (thisNode._astname === "Call") {
            ...
            let isListFunc; let isStrFunc = false
    ...
    function recursivelyGetListValues(node: any, combinedList: any[]) {
        ...
        if (leftNode != null) {
            ...
            if (leftNode._astname === "Name") {
                ...
                const udf = getVariableObject(leftNode.id.v)
...
export function allReturnsFilled() {
    ...
    for (let j = 0; j < ccState.getProperty("allVariables").length; j++) {
        ...
        if (ccState.getProperty("allVariables")[j].value === "List" && ccState.getProperty("allVariables")[j].containedValue != null) {
            ...
            for (const p in ccState.getProperty("allVariables")[j].assignedModified) {
