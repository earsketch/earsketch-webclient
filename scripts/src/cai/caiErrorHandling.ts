...
function handleError(error: any, codeObj: string){
	console.log(error);
	var errorType = String(error).split(':')[0] as keyof typeof errorFunctions
...
function handleNameError(error: any) {
	...
	var nameScores: [number, string, string][] = [];
	...
	var bestReplacement = sortedNames[0][2];
...
function updateNames(variables: any, functions: any) {
