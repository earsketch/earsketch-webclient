...
exports.generateResults = exports.randomNucleus = exports.storeHistory = exports.generateCodeSuggestion = exports.getMusic = void 0;
var codeRecommendations_1 = require("./codeRecommendations");
var caiProjectModel = require("./projectModel");
var complexityCalculatorHelperFunctions = require("./complexityCalculatorHelperFunctions");
var caiAnalysisModule = require("./analysis");
var caiErrorHandling = require("./errorHandling");
var currentDelta = { soundsAdded: [], soundsRemoved: [], sections: 0 };
var currentDeltaSum = 0;
var noDeltaCount = 0;
var currentResults = {};
var averageGenreThreshold = 0.8;
var musicResults = {};
...
var CAI_REC_DECISION_TREE = [
    ...
    },
    {
        ...
        condition: function () {
            ...
            possibleDeltaSuggs = [];
            for (var i in codeRecommendations_1.CAI_DELTA_LIBRARY) {
                // get current value and compare to end value
                var endValuesMatch = true;
                for (var j in codeRecommendations_1.CAI_DELTA_LIBRARY[i].end) {
                    if (codeRecommendations_1.CAI_DELTA_LIBRARY[i].end[j] !== currentResults[j]) {
                        endValuesMatch = false;
                    }
                }
                var startValuesMatch = true;
                if (endValuesMatch) {
                    for (var j in codeRecommendations_1.CAI_DELTA_LIBRARY[i].start) {
                        if (codeRecommendations_1.CAI_DELTA_LIBRARY[i].start[j] !== (currentResults[j] - currentDelta[j])) {
                ...
                if (endValuesMatch && startValuesMatch) {
                    deltaInLib = true;
                    possibleDeltaSuggs.push(codeRecommendations_1.CAI_DELTA_LIBRARY[i]);
                }
            }
            return deltaInLib;
        },
...
function getSuggestionByID(suggID) {
    for (var i in codeRecommendations_1.CAI_RECOMMENDATIONS) {
        if (codeRecommendations_1.CAI_RECOMMENDATIONS[i].id === suggID) {
            var suggestion = Object.assign({}, codeRecommendations_1.CAI_RECOMMENDATIONS[i]);
            return suggestion;
        }
    }
}
...
function generateCodeSuggestion(history) {
    var nodeIndex = 0;
...
exports.generateCodeSuggestion = generateCodeSuggestion;
function storeHistory(historyList) {
    storedHistory = historyList;
}
exports.storeHistory = storeHistory;
function randomNucleus(history, suppressRepetition) {
    if (history === void 0) { history = {}; }
    if (suppressRepetition === void 0) { suppressRepetition = true; }
    var isAlreadySaid = true;
    var newNucleus = { utterance: "" };
    var threshold = 10;
    while (isAlreadySaid) {
        threshold -= 1;
        if (threshold < 0) {
        ...
        newNucleus = codeRecommendations_1.CAI_NUCLEI[getRandomInt(0, codeRecommendations_1.CAI_NUCLEI.length - 1)];
        isAlreadySaid = false;
...
exports.randomNucleus = randomNucleus;
function generateResults(text, lang) {
    ...
    if (!isEmpty(currentResults)) {
        ...
        if (validChange && prevScore > 0 && somethingChanged) {
            ...
            var keys_1 = Object.keys(codeDelta);
            for (var i in keys_1) {
                codeDelta[keys_1[i]] -= currentResults[keys_1[i]];
            }
...
//# sourceMappingURL=codeSuggestion.js.map