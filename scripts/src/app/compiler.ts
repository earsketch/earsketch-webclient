...
function handleUndefinedNamesPY(code: string, bypass: boolean) {
    ...
    return Promise.all(undefinedNames.map(function (name) { return audioLibrary.verifyClip(name); })).then(function (result) {
...
function throwErrorWithLineNumber(error: Error | string, lineNumber: number) {
    ...
    if (typeof (error) === 'string') {
        if (lineNumber) {
            err = new EvalError(error + ' on line ' + lineNumber);
            (err as any).lineNumber = lineNumber;
        } else {
            err = new EvalError(error);
        }
        throw err;
    } else {
        if (lineNumber) {
...
function getClipTempo(result: DAWData) {
    ...
    var tempoCache: { [key: string]: number } = {};
...
function loadBuffers(result: DAWData) {
    ...
    return Promise.all(promises).then(function(buffers) {
        var map: { [key: string]: AudioBuffer } = {};
...
function addMetronome(result: DAWData) {
    ...
    function(r) {
        var track = {clips:[] as Clip[],effects:[], analyser: null as AnalyserNode | null};
