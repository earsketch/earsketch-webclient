...
export function show(text: string, type: string="", ...args: any[]) {
    ...
    if (['bell','popup','history'].indexOf(type)) { // temporary tags for bell-icon dropdown notifications
        ...
        while (history.some(function (item) {
    ...
    callbacks.popup.apply(this, [text, type, ...args])
}

export const showBanner = (...args: any[]) => callbacks.show.apply(this, args)
...
function truncateBroadcast() {
    var nonBroadcasts = history.filter(function (v) {
        return v.notification_type !== 'broadcast'
    })

    var latestBroadcast = history.filter(function (v) {
        return v.notification_type === 'broadcast'
    })[0]
...
export const handleTeacherBroadcast = (data: any) => {
