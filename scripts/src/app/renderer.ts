 ...
 */
 export function renderBuffer(result: DAWData) {
 ...
 */
 export function renderWav(result: DAWData) {
     return new Promise(function(resolve, reject) {
         renderBuffer(result).then(function(buffer: AudioBuffer) {
  ...
  * @returns {Promise} A promise that resolves to a Blob object.
  */
 export function renderMp3(result: DAWData) {
     return new Promise(function (resolve, reject) {
         renderBuffer(result).then(function (buffer: AudioBuffer) {
  ...
  */
 export function mergeClips(clips: Clip[], tempo: number) {
     esconsole('Merging clips', ['DEBUG', 'RENDERER']);
     // calculate the length of the merged clips
     let length = 0;
     for (let i = 0; i < clips.length; i++) {
         const end = clips[i].measure + clips[i].end;
         if (end > length) {
             length = end;
         }
     }
