...
import { AudioContextWithGain } from '../app/player'
...
const VolumeNode = function (context: AudioContext) {
    ...
    this.connect = output.connect
};

const DelayNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const FilterNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const CompressorNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const PannerNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const BandpassNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const Eq3bandNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const ChorusNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const FlangerNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const PhaserNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const TremoloNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
const DistortionNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    ...
    this.connect = this.output.connect
};

//------------------------- PITCHSHIFT ----------------------------------//
const PitchshiftNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();

    this.connect = this.input.connect
};

//------------------------- RING MODULATOR ----------------------------------//
var RingmodNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
...
//------------------------- WAH-WAH ----------------------------------//
var WahNode = function (context: AudioContext) {
    //create the nodes we’ll use
    this.input = context.createGain();
    var output = context.createGain();
    this.bandpass = context.createBiquadFilter();
...
//------------------------- REVERB -------------------------//
var ReverbNode1 = function (context: AudioContext) {
    this.input = context.createGain();
...
function Freeverb (audioContext: AudioContext) {
    var node: GainNode & {combFilters?: AudioNode[]} = audioContext.createGain()
...
function LowpassCombFilter (context: AudioContext) {
    ...
    var node: DelayNode & {dampening?: AudioParam, resonance?: AudioParam} = context.createDelay(1)
...
function bypassValueComplement(bypass_state: number) {
    if (bypass_state)
        return 0;
    else
        return 1;
...
interface BuildAudioNodeGraph {
    (context: AudioContext, track: any, tracknumber: number, tempo: number, offsetInSeconds: number, master_collect: any, bypassedEffects: any, wav_export: any): any
...
export const buildAudioNodeGraph: BuildAudioNodeGraph = <BuildAudioNodeGraph>((context: AudioContextWithGain, track: any, tracknumber: number, tempo: number, offsetInSeconds: number, master_collect: any, bypassedEffects: any, wav_export: any) => {
    /**
     ...
     * @returns {number}
     */
    function getOffsetTime(location: number) {
     ...
     * @param time
     */
    function setMix(node: any, wetValue: number | "default", time: number) {
        if (wetValue === 'default') {
     ...
     * @param node {object} EffectNode
     */
    function setMixInTheFuture(node: any) {
     ...
     * @param param
     */
    function applyEffectInTheFuture(param: AudioParam) {
     ...
     */
    function staircaseRamp(callback: (a: number) => void, interval: number, sync: boolean) {
        ...
        function oscTimer(cb: () => void, time: number) {
    ...
    for (var i in effects) {
        ...
        if (effect.name == 'VOLUME') {
            ...
            if (buildAudioNodeGraph.gainNodeCreatedFlag == 0) {
                var VolumeEffectNode = new (VolumeNode as any)(context);
        ...
        if (effect.name == 'DELAY') {
            ...
            if (buildAudioNodeGraph.delayNodeCreatedFlag == 0) {
                var DelayEffectNode = new (DelayNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'FILTER') {
            ...
            if (buildAudioNodeGraph.filterNodeCreatedFlag == 0) {
                var FilterEffectNode = new (FilterNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'BANDPASS') {
            ...
            if (buildAudioNodeGraph.bandpassNodeCreatedFlag == 0) {
                var BandpassEffectNode = new (BandpassNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'EQ3BAND') {
            ...
            if (buildAudioNodeGraph.eq3bandNodeCreatedFlag == 0) {
                var Eq3bandEffectNode = new (Eq3bandNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'COMPRESSOR') {
            ...
            if (buildAudioNodeGraph.compressorNodeCreatedFlag == 0) {
                var CompressorEffectNode = new (CompressorNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'CHORUS') {
            ...
            if (buildAudioNodeGraph.chorusNodeCreatedFlag == 0) {
                var ChorusEffectNode = new (ChorusNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'FLANGER') {
            ...
            if (buildAudioNodeGraph.flangerNodeCreatedFlag == 0) {
                var FlangerEffectNode = new (FlangerNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'PHASER') {
            ...
            if (buildAudioNodeGraph.phaserNodeCreatedFlag == 0) {
                var PhaserEffectNode = new (PhaserNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'TREMOLO') {
            ...
            if (buildAudioNodeGraph.tremoloNodeCreatedFlag == 0) {
                var TremoloEffectNode = new (TremoloNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'PAN') {
            ...
            if (buildAudioNodeGraph.pannerNodeCreatedFlag == 0) {
                var PannerEffectNode = new (PannerNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'DISTORTION') {
            ...
            if (buildAudioNodeGraph.distortionNodeCreatedFlag == 0) {
                var DistortionEffectNode = new (DistortionNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'PITCHSHIFT') {
            ...
            if (buildAudioNodeGraph.pitchshiftNodeCreatedFlag == 0) {
                var PitchshiftEffectNode = new (PitchshiftNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'RINGMOD') {
            ...
            if (buildAudioNodeGraph.ringmodNodeCreatedFlag == 0) {
                var RingmodEffectNode = new (RingmodNode as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag != 0) {
        ...
        if (effect.name == 'WAH') {
            ...
            if (buildAudioNodeGraph.wahNodeCreatedFlag == 0) {
                var WahEffectNode = new (WahNode as any)(context);
        ...
        if (effect.name == 'REVERB') {
            if (buildAudioNodeGraph.reverbNodeCreatedFlag === 0) {
                var ReverbEffectNode1 = new (ReverbNode1 as any)(context);
                if (buildAudioNodeGraph.firstNodeCreatedFlag !== 0) {
