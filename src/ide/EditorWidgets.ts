...
class SoundPreviewWidget extends WidgetType {
    ...
    toDOM() {
        ...
        previewButton.onclick = () => {
            store.dispatch(soundsThunks.preview({ name: this.name }))
        }
        return wrap
    }
...
function previews(view: EditorView, soundNames: string[], soundPreview: Preview) {
        ...
        syntaxTree(view.state).iterate({
            ...
            enter: (node) => {
                if (node.name === "VariableName") {
                    ...
                    if (isSoundConstant) {
                        const state = soundPreview?.value === name
