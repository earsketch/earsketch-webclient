...
const suggestionContent: SuggestionContent = {
    0: { id: 200, utterance: "why don't we use [LINK|variables] to name our sounds so it's easier to swap in new sounds?", explain: "storing our sound samples in a variable lets us use that as a name, and it makes it easier to try different sounds", example: "instead of\n\nfitMedia(HIPHOP_SYNTHPLUCKLEAD_005, 1, 1, 2)\nfitMedia(HIPHOP_SYNTHPLUCKLEAD_005, 1, 3, 4)\n\nwe could write\n\nsynth1 = HIPHOP_SYNTHPLUCKLEAD_005\nfitMedia(synth1, 1, 1, 2)\nfitMedia(synth1, 1, 3, 4)" },
    ...
    2: { id: 202, utterance: "using a [LINK|for loop] can help us repeat code without having to write it a bunch of times", explain: "for loops (or while loops!) can run code multiple times, which makes it easier for us to put in a bunch of beats or samples", example: "for example, this code:\n\nfor measure in range(1, 5):\n    makeBeat(OS_SNARE03, 1, measure, \"0---0---0-0-0---\")\n\nputs the same beat in measures 1-5 on track 1" },
    5: { id: 205, utterance: "we can use a [LINK|conditional statement] for mixing or to alternate beats in different measures", explain: "[LINK|conditional statement]s let us make EarSketch do different things if a statement is TRUE or FALSE", example: "loudnessTrack1 = analyzeTrack(1, RMS_AMPLITUDE)\nloudnessTrack2 = analyzeTrack(2, RMS_AMPLITUDE)\n\n# If track 1 is louder than track 2, we reduce its volume\nif loudnessTrack1 > loudnessTrack2:\n    setEffect(1, VOLUME, GAIN, -10)" },
    8: { id: 208, utterance: "a custom [LINK|function] can help us write code that we can call again and again", explain: "[LINK|custom function]s can help us make sections of music without having to write too many fitMedia() calls", example: "def sectionA(start, end):\n    fitMedia(melody2, 1, start, end)\n    fitMedia(brass1, 2, start, end)\n    setEffect(2, VOLUME, GAIN, -20, start, -10, end)\nsectionA(1,5)" },
    10: { id: 210, utterance: "we can also use [LINK|function]s to return a value, and use that value somewhere else", explain: "returning values lets us use the outputs of functions in other places", example: "def createBeat(startMeasure, soundClip, beatString):\n    endMeasure = startMeasure + 3\n    for measure in range(startMeasure, endMeasure):\n        makeBeat(soundClip, 1, measure, beatString)\n\n    # Return ending measure so we can use it outside function\n    return endMeasure" },
    11: { id: 211, utterance: "we could let the user control something about our song by using [LINK|console input]", explain: "using readInput() lets us ask the user for input, which we can then pair with a [LINK|conditional statement] to change things about our song based on what the user inputs", example: "clipNumber = readInput(\"Type a clip number between 10 and 46: \")\ndubstepClip = \"DUBSTEP_BASS_WOBBLE_0\"\n    finalClip = dubstepClip + clipNumber\n\n    fitMedia(finalClip, 1, 1, 5)" },
    13: { id: 213, utterance: "using a list with indexing, we could use a version of [LINK|makeBeat] that includes different sounds in the beat", explain: "using the advanced [LINK|makeBeat]() lets us make a beat with as many different sounds as we want", example: "drums = [OS_KICK05, OS_SNARE01]\n    beat = \"0+++1+++0+++1+++\"\n    makeBeat(drums, 1, 3, beat)" },
}
