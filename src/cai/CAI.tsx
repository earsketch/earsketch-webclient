...
const CaiMessageView = ({ message }: { message: cai.CaiMessage }) => {
    ...
    const wholeMessage = message.text.map((phrase: [string, string], index) => {
        switch (phrase[0]) {
            ...
            case "LINK":
                return <a key={index} className="hover:text-yellow-500 text-blue-500 underline" href="#" onClick={e => { e.preventDefault(); dispatch(curriculum.openCurriculum(phrase[1][1], dispatch)); addToNodeHistory(["curriculum", phrase[1][1]]) }}>{phrase[1][0]}</a>
            case "sound_rec":
