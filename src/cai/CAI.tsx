...
export const CaiBody = () => {
    ...
    return (
        <div id="cai-body">
            {FLAGS.SHOW_CAI &&
                <div>
                    <video ref={vidRef} src={FLAGS.SHOW_NLU ? "https://earsketch.gatech.edu/videoMedia/cai-nlu-v1.mp4" : "https://earsketch.gatech.edu/videoMedia/cai_denoise.mp4"} controls style={{ width: "100%", maxWidth: "webkit-fill-available" }} onClick={onPlayPress}></video>
                </div>}
            <div className="chat-message-container text-sm">
                <ul>
                    {messages &&
                    messages.map((message: cai.CAIMessage, idx) =>
                        <li key={idx}>
            ...
            <div className={`grid grid-cols-${cols} gap-2`}>
                {Object.entries(options).map(([_, input]: [string, number]) =>
                    <div key={options.indexOf(input)}>
                        <button className={caiButtonCSS} title={CAI_TREE_NODES[input].title} onClick={() => [dispatch(caiThunks.sendCAIMessage([{ label: CAI_TREE_NODES[input].title, value: String(input) }, true, true])), setActiveSubmenu(null)]}>{CAI_TREE_NODES[input].title}</button>
                    </div>
                )}
            </div>
        </>
                            ...
                            {!dropupLabel.length
                                ...
                                : <div className="list-cai-content">
                                    <ul className="min-w-full">
                                        {Object.entries(inputOptions).map(([inputIdx, input]: [string, cai.CAIButton]) =>
                                            <li key={inputIdx}>
                                                <button className="btn break-all text-left w-full" title={input.label} onClick={() => dispatch(caiThunks.sendCAIMessage([input, false, true]))}>{input.label}</button>
                                            </li>)}
                                    </ul>
                                </div>}
                        </div>}
