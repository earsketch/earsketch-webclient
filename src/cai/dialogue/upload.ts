...
async function uploadCaiHistory(project: string, node: any, sourceCode?: string) {
    ...
    if (import.meta.env.SHOW_CAI) {
        if (import.meta.env.SHOW_CHAT) {
            data.ui = "Wizard"
        } else {
            data.ui = "CAI"
        }
    } else if (import.meta.env.SHOW_CHAT) {
        data.ui = "Chat"
    }
...
export function addToNodeHistory(nodeObj: any, sourceCode?: string, project?: string) {
    ...
    if ((import.meta.env.SHOW_CAI || import.meta.env.SHOW_CHAT || import.meta.env.UPLOAD_CAI_HISTORY) && state[project] && state[project].nodeHistory) {
        state[project].nodeHistory.push(nodeObj)
        if (import.meta.env.UPLOAD_CAI_HISTORY && nodeObj[0] !== 0) {
