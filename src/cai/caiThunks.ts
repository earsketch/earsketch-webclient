...
const addCaiMessage = createAsyncThunk<void, [CaiMessage, MessageParameters], ThunkAPI>(
    ...
    ([message, parameters], { getState, dispatch }) => {
        if (!import.meta.env.VITE_SHOW_CHAT || message.sender !== "CAI") {
...
export const caiSwapTab = createAsyncThunk<void, string, ThunkAPI>(
    ...
    (activeProject, { getState, dispatch }) => {
        if (!activeProject || activeProject === "") {
            ...
            if ((import.meta.env.VITE_SHOW_CAI || import.meta.env.VITE_UPLOAD_CAI_HISTORY) && !selectWizard(getState())) {
            ...
            if (!selectMessageList(getState()).length) {
                ...
                if (import.meta.env.VITE_SHOW_CAI && !selectWizard(getState())) {
...
export const compileCai = createAsyncThunk<void, [DAWData, Language, string], ThunkAPI>(
    ...
    async (data, { getState, dispatch }) => {
        ...
        if (import.meta.env.VITE_SHOW_CAI) {
            ...
            dispatch(autoScrollCai())
            newCaiMessage()
        } else if (import.meta.env.VITE_UPLOAD_CAI_HISTORY) {
...
export const compileError = createAsyncThunk<void, string | Error, ThunkAPI>(
    ...
    (data, { getState, dispatch }) => {
        ...
        if (import.meta.env.VITE_SHOW_CAI && import.meta.env.VITE_SHOW_CHAT && !selectWizard(getState())) {
            const message = {
...
export const closeCurriculum = createAsyncThunk<void, void, ThunkAPI>(
    "cai/closeCurriculum",
    (_, { getState }) => {
        if (import.meta.env.VITE_SHOW_CAI && import.meta.env.VITE_SHOW_CHAT && !selectWizard(store.getState())) {
            sendChatMessage({
