...
const Clip = ({ color, clip }: { color: daw.Color, clip: types.Clip }) => {
    ...
    return <button
        ...
        title={scriptMatchesDAW ? `Line: ${clip.sourceLine}` : t("daw.needsSync")} role="navigation" aria-label={`${clip.filekey} from measure ${clip.clipFamilyStart || (clip.measure + clip.start - 1)} to ${clip.clipFamilyEnd || (clip.measure + clip.end - 1)}`}
        onClick={() => { player.setPreview(clip.track); player.play(clip.clipFamilyStart || (clip.measure + clip.start - 1), 0, clip.clipFamilyEnd || (clip.measure + clip.end - 1)) }}
    >
